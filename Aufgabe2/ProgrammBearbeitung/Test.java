public class Test {
	...
	public static void main(String[] args) throws Exception {
		...
		double b[] = { 34, 12 , -6 , 4};
		double C[][] = { { 1, 4 , 5 , 0 }, { 3, 2 , 4 , 7 } ,{ 6, 2 , 0 , 3}, { 1, 0 , 0 , 44} }; //reguläre
		double A[][] = { { 1, 2 ,3}, { 4, 5 ,6 }, { 7, 8 ,9 } }; //singuläre
		double xC[] = { 1, 1 };
		double xA[] = { 1, -1 };
		double x[];
		...
		if (test_gauss) {
			...
			x = Gauss.solve(C, b);
                        //Lsg: -3,50000 ; 7,244318 ; 1,704545 ; 0,170455
			if (Util.vectorCompare(x, xC)) {
				System.out.println("    Richtiges Ergebnis");
			} else {
				System.out.println("    FEHLER: falsches Ergebnis:");
			...
			x = Gauss.solveSing(A);
                        Util.printVector(x);
                        Util.printVector(Gauss.matrixVectorMult(A,x));
		...
		if (test_pagerank) {
			 ...
			 */
			lm.read("C:\\Users\\Andreas\\Documents\\NetBeansProjects\\NumProg\\src\\irgendwo.txt");
